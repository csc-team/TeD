...
int getComp(IplImage *in, CvRect** _regions) {
    ...
    (*_regions) = new CvRect[darkCompBB.size() + lightCompBB.size()];
    CvRect* regions = (*_regions);

    int num = 0;

    //std::vector< CvRect > darkCompLines;
    //std::vector< CvRect > lightCompLines;
   ...
   // CvRect r = getRegion(regions, num, in);
...
bool addRectToLine(std::pair<CvRect, int> &compLines, CvRect rect) {

	CvRect r0 = compLines.first;
	
	if ( (intersect (r0.y, r0.y + r0.height, rect.y, rect.y + rect.height)) && (abs(rect.x - r0.x - r0.width) < 2 * rect.height) &&
		 (abs(rect.y - r0.y) < rect.height) && (abs(rect.height - r0.height + 0.0) / (r0.height + 0.0) < 0.3)  ) {

		compLines.second++;

		if (r0.x > rect.x) compLines.first.x = rect.x;
		if (r0.y > rect.y) compLines.first.y = rect.y;
		if (r0.x + r0.width < rect.x + rect.width) compLines.first.width = rect.x + rect.width - compLines.first.x;
		if (r0.y + r0.height < rect.y + rect.height) compLines.first.height = rect.y + rect.height - compLines.first.y;
		return true;
	}
...
bool addRectToAreaHor(std::pair<CvRect, int> &compLines0, std::pair<CvRect, int> &compLines1) {

	CvRect r0 = compLines0.first;
	CvRect r1 = compLines1.first;
	
	if ( 
		 (intersect (r0.y, r0.y + r0.height, r1.y, r1.y + r1.height) )
		  && (abs(r1.x - r0.x - r0.width) < 3 * r1.width ) 
		  && (abs(r1.y - r0.y) < 2 * r1.height)
		 && (abs(r1.height - r0.height  + 0.0) < r1.height)
	) {

		compLines0.second += compLines1.second;
		compLines1.second = 0;

		if (r0.x > r1.x) compLines0.first.x = r1.x;
		if (r0.y > r1.y) compLines0.first.y = r1.y;
		if (r0.x + r0.width < r1.x + r1.width) compLines0.first.width = r1.x + r1.width - compLines0.first.x;
		if (r0.y + r0.height < r1.y + r1.height) compLines0.first.height = r1.y + r1.height - compLines0.first.y;
		return true;
	}
...
bool addRectToAreaVer(std::pair<CvRect, int> &compLines0, std::pair<CvRect, int> &compLines1) {

	CvRect r0 = compLines0.first;
	CvRect r1 = compLines1.first;
	
	if ( 
		   (intersect (r0.x, r0.x + r0.width, r1.x, r1.x + r1.width)) 
		//&& (abs(r1.width - r0.width) < abs(r1.y - r0.y)) 
		 && (abs(r1.y - r0.y - r0.height) < 2 * r1.height)
		 && (abs(r1.height - r0.height / (compLines0.second + 0.0)) < 2 * r1.height)
	) {
...
CvRect getRegion(CvRect* out, int len) {
    ...
    std::list< std::pair<CvRect, int> > darkCompLines;
    std::list< std::pair<CvRect, int> > lightCompLines;

    int i = 1;

    //make lines
    while ((out[i].y >= out[i - 1].y) && (i < len)) {
	
	int k = 0;
	for (std::list< std::pair<CvRect, int> >::iterator it = darkCompLines.begin(); it != darkCompLines.end(); ++it) {
		if (((*it).second == 1) && ((*it).first.y + (*it).first.height < out[i].y)) {
			it = darkCompLines.erase(it);
		}

		if  (addRectToLine((*it), out[i])) k++;
	}
	if (k == 0) {
		std::pair<CvRect, int> pair(out[i], 1);
	...
	for (std::list< std::pair<CvRect, int> >::iterator it = darkCompLines.begin(); it != darkCompLines.end(); ++it) {
		if ((*it).second == 1) it = darkCompLines.erase(it);
	}

    while (i < len) {

	int k = 0;
	for (std::list< std::pair<CvRect, int> >::iterator it = lightCompLines.begin(); it != lightCompLines.end(); ++it) {
		if (((*it).second == 1) && ((*it).first.y + (*it).first.height < out[i].y)) {
			it = lightCompLines.erase(it);
		}
	...
	for (std::list< std::pair<CvRect, int> >::iterator it = lightCompLines.begin(); it != lightCompLines.end(); ++it) {	
		if ((*it).second == 1) it = lightCompLines.erase(it);
	}
   //make areas


    for (std::list< std::pair<CvRect, int> >::iterator it = darkCompLines.begin(); it != darkCompLines.end(); ++it) {
	std::list< std::pair<CvRect, int> >::iterator prev = it;
	++prev;
        for (; prev != darkCompLines.end(); ++prev) {
		if  ((*prev).second != 0) addRectToAreaHor( (*it), (*prev));
        }
    }

	for (std::list< std::pair<CvRect, int> >::iterator it = darkCompLines.begin(); it != darkCompLines.end(); ++it) {
		if ((*it).second == 0) it = darkCompLines.erase(it);
		else (*it).second = 1;
	}
	...
	std::list< std::pair<CvRect, int> >::iterator prev = it;
	++prev;
        for (; prev != lightCompLines.end(); ++prev) {
		if  ((*prev).second != 0) addRectToAreaHor( (*it), (*prev));
	...
	for (std::list< std::pair<CvRect, int> >::iterator it = lightCompLines.begin(); it != lightCompLines.end(); ++it) {	
		if ((*it).second == 0) it = lightCompLines.erase(it);
		else (*it).second = 1;
	}
	...
	++prev;
        for (; prev != darkCompLines.end(); ++prev) {
		if  ((*prev).second != 0) addRectToAreaVer( (*it), (*prev));
        }
    }
    ...
    for (std::list< std::pair<CvRect, int> >::iterator it = darkCompLines.begin(); it != darkCompLines.end(); ++it) {
		...
		if  ((*it).first.height * (*it).first.width > maxArea) {
			maxArea = (*it).first.height * (*it).first.width;
			r = (*it).first;
		}
    }
